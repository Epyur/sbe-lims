...
- Нумерация (см. дизайн-спеку §9): сервер присваивает глобальный по году NNN
  ...
  Номера резервируются при создании/добавлении метода. **Пересчитывается ровно
  один случай (2026-09-21): смена проекта у заявки** — тогда `customer_number`
  пересобирается под новый код проекта (`rebuildCustomerNumber`, зовётся из
  `handleUpdateRequest` и `pushUpdate`). `lab_number` не меняется никогда:
  кода проекта в нём нет.
...
## Страница со списком ручек (Swagger, 2026-09-21)

`GET /api/lab/docs/` — Swagger UI, `GET /api/lab/openapi.yaml` — схема. Обе
открыты без ключа: данных в них нет, только описание ручек.

- Дистрибутив UI и схема вшиты в бинарь (`static.go`, `//go:embed`), без CDN —
  сервис обязан работать без внешней сети. Лицензия Apache-2.0.
- **Пути обязательно внутри `/api/lab/`**: Caddy проксирует сервису только этот
  префикс, корневой `/docs/` снаружи не откроется. Образец, с которого это
  снято, отдаёт UI по корневому пути — скопировать его как есть было бы
  граблями.
- `Dockerfile` копирует в контекст перечисленные пути, поэтому там нужна строка
  `COPY static ./static`; без неё сборка падает на `pattern static/...: no
  matching files found`.
- **Схема правится в том же коммите, что и ручка.** Расхождение ловит
  `scripts/check_lab_openapi.mjs`: берёт маршруты из `main.go`, сверяет с
  путями и методами схемы в обе стороны (ручка без описания и описание без
  ручки одинаково плохи), плюс структурная проверка файла. Запуск —
  `node scripts/check_lab_openapi.mjs` из папки плагинов.
- Где тело ответа описано свободным `object` — форма намеренно не
  зафиксирована: её задаёт конфигурация метода испытаний. Выдумывать точный вид
  хуже, чем честно сказать, что он переменный.
...
| PATCH | `/requests/{id}` | editor+/владелец (смена `project_id` пересобирает `customer_number` — 2026-09-21) |
...
## Фикс: легенда графика с несколькими сериями (2026-08-29)

Живая жалоба пользователя: на графике `kind="timeseries"` (например, «Температура
дымовых газов и скорость нарастания», метод ГГ) при НЕСКОЛЬКИХ сериях
эксперимента с сырой кривой прибора (`smoke_temp_curve` и т.п.) каждая серия
рисует свою кривую того же канала — `buildChartSeriesFromTimeseries` давал
ВСЕМ кривым одного канала ОДНУ И ТУ ЖЕ подпись легенды (`label` из
`timeseries_series`-конфига, без разбивки по сериям) — на графике кривые
неразличимы, кроме как по цвету без объяснения, какая есть какая.

- `results.go`: новый `loadSeriesValuesWithSeriesNum` + тип `seriesValuesRow
  {SeriesNum int; Values map[string]any}` — параллельно существующему
  `loadSeriesValues` (НЕ замена — та используется в 5+ местах, где
  `series_num` не нужен, менять её сигнатуру ради одного вызывающего было бы
  лишним риском регрессии).
- `charts.go`: `buildChartSeries`/`buildChartSeriesFromTimeseries` теперь
  принимают `[]seriesValuesRow` вместо `[]map[string]any`. При >1 серии в
  `buildChartSeriesFromTimeseries` подпись каждой кривой дополняется номером
  серии (`"Канал 1 (серия 2)"`); при ровно одной серии — без изменений (не
  загромождать легенду, если различать нечего).
- `protocol.go`: `placeholderCtx` += параллельное поле `seriesNums []int`
  (тот же принцип, что уже есть у `photoBefore`/`photoAfter` — тот же индекс
  = та же серия), заполняется вместе с `series` в один проход (без лишнего
  запроса к БД). Новый `seriesValuesRowsFromCtx(ctx)` — сборка `[]
  seriesValuesRow` для графика внутри блока протокола
  (`renderBlockChartHTML`).
- Тесты: `TestBuildChartSeriesFromTimeseriesMultiSeriesLabels` (2 серии → 2
  разные подписи), `TestBuildChartSeriesFromTimeseriesSingleSeriesNoSuffix`
  (1 серия → подпись БЕЗ номера, защита от регрессии «стало добавлять номер
  всегда»); остальные существующие тесты `charts_test.go` адаптированы под
  новую сигнатуру (`[]seriesValuesRow` вместо `[]map[string]any]`), поведение
  не менялось.
- **Живой E2E**: запрос протокола реальной заявки 1339 (метод ГГ, 3 серии с
  `smoke_temp_curve`, реальный chart_config `smoke_temp_chart`) —
  HTML отдался без ошибок, изображение графика встроено, заголовок графика
  на месте.
- `go build`/`go vet`/`go test` — чисто. Backend деплой обязателен
  (`charts.go`/`results.go`/`protocol.go`), выполнен, health ok.
- Клиент не менялся — легенда целиком формируется на сервере (текст впечатан
  в PNG).
...
- **2026-09-21 — номер заказчику пересчитывается при смене проекта.** Жалоба
  пользователя: проект у заявки меняешь, а код проекта в номере остаётся
  прежним. Причина одна на оба клиента — номер собирается только при создании
  (`buildNumbers`), а оба пути правки (`handleUpdateRequest` для веба,
  `pushUpdate` для плагина) меняли `project_id` и номер не трогали: пересчитывать
  его было попросту некому. Заведена `rebuildCustomerNumber`, зовётся из обоих
  путей, когда проект действительно изменился; `pushUpdate` ради этого обёрнут
  в транзакцию. `lab_number` не трогаем — кода проекта в нём нет.
  - **Коды метода и лабы читаются напрямую по id, без `loadMethodLabRow`.** Пара
    зафиксирована при создании заявки, и если связь в `method_labs` потом убрали
    из справочника, номер обязан пересобраться, а не потеряться на проверке.
  - **Отметки времени заявок и справочников pull переведены на `RFC3339Nano`**
    (15 мест в `requests.go`, `sync.go`, `projects.go`). Без этого пересчёт не
    доехал бы до людей вовсе: клиент хранит `updated_at` с миллисекундами, а
    усечённая до секунд серверная отметка проигрывает LWW-слияние собственной
    же записи клиента. Та же ошибка уже ловилась на `mailer-service` и
    `documents-service` 2026-09-19 — здесь она была третьей по счёту. Остальные
    места сервиса (`Format(time.RFC3339)` ещё примерно в двух десятках) не
    трогали: править по мере правки каждого пути.
  - **Попутно найдена и исправлена вторая ошибка.** В `handleUpdateRequest`
    вычислялся `effProjectID` (включая создание ЕКН-проекта через
    `ensureEknProject`), а в `UPDATE` подставлялся `req.ProjectID`. То есть при
    пустом проекте и заданном ЕКН проект заводился в базе и тут же терялся —
    заявка оставалась без него. Никем не заявлена, найдена при разборе.
  - **`loadVisibleProjects` больше не путает «не видно» и «не смогли
    проверить».** Ошибка запроса членства в группе возвращала `false`, то есть
    сбой базы отдавал клиенту пустой список проектов как правду; плагин затирал
    им свой кэш, и в форме заявки было нечего выбрать до следующего удачного
    pull. Теперь ошибка идёт наружу и pull честно отказывает — прежний кэш у
    клиента остаётся на месте.
  - **`mapEmailPriority` принимает шесть слов вместо трёх.** К словам
    легаси-трекера («Обычный», «Критичный», «Блокер») добавлены «Средний»,
    «Критический», «Блокирующий» — подписи важности в интерфейсах
    стандартизируются, а шаблоны писем трекера мы не меняем, поэтому старые
    слова остаются навсегда.
  - Новый `requests_test.go`: формат `buildNumbers` (страховка от расхождения с
    двумя копиями формулы на TypeScript — `sbe-core/src/utils/request-number.ts`
    и копия в портале, их между собой сверяет
    `scripts/check_request_number.mjs`), инвариант «смена проекта меняет только
    номер заказчику» и таблица слов `mapEmailPriority`.
  - Дизайн — `docs/superpowers/specs/2026-09-21-requests-edit-and-priority-filter-design.md`,
    план — одноимённый в `plans/`. **Задеплоено 2026-09-21** скриптом
    `scripts/lab_deploy.sh` (новый, по образцу `mailer_deploy.sh`): контейнер
    `lab` пересобран, health OK, закрытая ручка отвечает 401, в бинаре есть и
    `rebuildCustomerNumber`, и шаблон `RFC3339Nano`. Откат — `lab-service.bak`
    рядом. Живой прогон со сменой проекта на боевых данных ещё не делался.

- **2026-09-21 — ⚠️ реальная потеря данных: сохранение формы стёрло данные
  прибора.** Заявка 1578 (`PVC SAMPLES-468/2026-ЛПИ-ГГ`), серия 1. В 14:02:06
  UTC сохранение формы испытателя заменило `values` целиком и унесло двенадцать
  приборных полей: `tp1_smog`…`tp4_smog`, `temp_of_smog`, `time_of_max_temp`,
  `smoke_temp_curve`, `num_channels`, `pre/main/post_time_seconds`,
  `poll_interval_seconds`.
  - **Почему именно тогда.** Это был первый боевой прогон приборного буфера
    (`instrument_result_buffer`, QR-хэш). Сервер подмешивает значения буфера в
    серию В МОМЕНТ сохранения (`claimInstrumentBuffer` в `handleCreateResult`),
    а модель формы у клиента их не получает. Следующее сохранение из той же
    формы пришло без них — и `saveResultSeries`, который писал `values`
    целиком, их стёр.
  - **Почтовый путь этим не страдал**, и цифры это подтверждают: сохранений
    поверх уже имевшихся приборных полей — 183, поля уцелели в 182. С почтой
    данные уже лежат в серии, когда приложение её открывает: форма загружает
    их и отправляет обратно.
  - **Версия «повторно отправили тот же хэш» проверена и отпадает.** При
    повторном хэше `claimInstrumentBuffer` не находит строку с
    `consumed_at IS NULL`, и обработчик отвечает 400 ДО сохранения. Тогда не
    было бы ни записи в журнале, ни затирки. Запись есть — значит второе
    сохранение пришло без хэша.
  - **Правка:** `carryOverUnownedValues` — поля, которых нет во входящем наборе
    и которыми не распоряжается форма испытателя метода
    (`operatorFormAttributeIDs`), переносятся из сохранённой серии. Перенос
    идёт ДО `applyFormulas`, чтобы производные считались по полному набору.
    Сплошное слияние не годится: поле, которое форма показывает, испытатель
    вправе очистить. Состав формы не прочитался — переносим всё недостающее:
    лишнее поле чинится повторным вводом, потерянные данные прибора — нет.
    Тесты — `results_carryover_test.go`. **Задеплоено и запушено 2026-09-21**
    (`scripts/lab_deploy.sh`, ветка `backend`), наличие правки в бинаре
    проверено; восстановленные данные заявки 1578 перезапуск пережили.
  - **Восстановлено** из снимка «до» в журнале изменений
    (`scripts/restore_1578_instrument_values.sql`); значения сверены с записью
    буфера, кривая дыма совпала побайтово, ручной ввод испытателя не тронут.
  - **Сегодняшняя раскатка ни при чём**: коммиты партии про номера трогали
    `requests.go`, `sync.go`, `projects.go`, `email_ingest.go`, `main.go`,
    `Dockerfile`, `static.go` — `results.go` и путь сохранения не тронуты.
    Совпадение по времени (контейнер поднялся в 13:54:42) проверено и
    объясняется тем, что испытатель в этот день впервые пользовался буфером.
  - **Ложная тревога при разборе, записана в назидание.** Мне показалось, что
    классификация сломана: её субъекты ссылаются на `avg_temp_of_smog` и
    `agg_comb_length`, а в статистической строке лежат `temp_of_smog_avg` и
    `average_comb_length_avg`, и ни одного выходного атрибута группы в строке
    нет. На деле расчёт исправен: агрегированные входы и выходы классификации
    **нигде не хранятся** — их считают на лету при чтении (см. комментарии про
    aggregated-only входы вроде `agg_flam_flow_density`, который «никогда не
    появляется» в per-series values). Проверка «ключа нет в базе» к ним
    неприменима: в базе их нет ни у одной из заявок, и это норма.

- **2026-09-21 — страница со списком ручек (Swagger).** Правило корневого
  AGENTS.md требует её «по мере очередного планового обновления сервиса»; у
  `lab-service` её не было, а правка номеров как раз такое обновление.
  Подробности — раздел «Страница со списком ручек» выше. Схема покрывает все 82
  операции; сверка с `main.go` — `scripts/check_lab_openapi.mjs`, проходит.
  Версия плагина не поднималась: правка чисто серверная. **Не задеплоено** —
  контейнер с этой правкой не пересобирался.

- **2026-09-12 — исполнитель назначается по факту ввода результатов.** Живая
  жалоба: заявка при первом сохранении серии сама переходила в «В работе» (так и
  задумано, 2026-09-05), но исполнитель оставался пустым, и назначать его
  приходилось вручную. Автопереход ставил ТОЛЬКО статус: `setRequestStatus`
  трогает `assigned_to` единственным способом — очищает при возврате в `new`.
  - Ломалось два места, а не одно. Карточка падала в ячейку «Не назначено»
    колонки «В работе», откуда испытатель забрать её не может (канбан разрешает
    двигать только свою карточку — `canApplyKanbanMove`), то есть требовался
    руководитель. И тем же переходом уходило письмо в LPITrack с текстом «взята
    в работу испытателем **не назначен**» (`triggerProcessingEmail` берёт
    исполнителя из `assigned_to`), а из-за дедупа по `sent_emails`
    (`lpitrack_processing`) исправленное письмо после ручного назначения уже НЕ
    отправляется — у заявок, прошедших этот путь до фикса, у получателя так и
    осталось «не назначен».
  - Фикс: `shouldAutoAssignToOperator` (чистая функция, как соседняя
    `shouldAutoTransitionToProcessing`) + `assignRequestTo` в `requests.go`,
    вызов — в `handleCreateResult` сразу после успешного сохранения серии.
    Исполнителем становится тот, кто ввёл результаты, если (а) исполнителя ещё
    нет — переназначение остаётся ручным делом руководителя, (б) вводящий
    действительно `lab_operator`/`lab_admin` этой лаборатории (та же проверка
    цели, что в `kanban-move`). Срабатывает и когда статус уже `processing`, но
    ячейка пустая (руководитель перевёл карточку, не назначив).
  - Порядок важен: назначение идёт ДО `setRequestStatus`, и `existing.AssignedTo`
    обновляется в памяти — иначе письмо в LPITrack снова ушло бы с «не назначен».
  - В самом UPDATE стоит условие «исполнитель пуст» — защита от гонки: чужое
    назначение, случившееся между проверкой и запросом, не затирается.
  - `go build`/`go vet`/`go test -count=1 ./...` — чисто, добавлен
    `TestShouldAutoAssignToOperator` (свой испытатель, админ лабы, аудитор,
    посторонний, уже назначенная другому и ему же). Версия плагина НЕ поднимается
    — правка чисто серверная.

- **2026-09-11 — группы полей формы испытателя: две строки структур и тест на
  сохранность схемы.** `MethodOperatorForm.Groups` (`json.RawMessage`) и
  `OperatorFormField.GroupID` — сервер группы НЕ интерпретирует, поля описаны
  ровно затем, чтобы `PATCH /methods/{id}` их не вырезал. Та же дисциплина, что
  у `Timer`/`Default`/`Visibility`, и та же причина: ключ, не описанный в
  Go-структуре, молча исчезает при пересохранении метода — дефект уже дважды
  доходил до прода.
  - Новый `TestParseMethodOperatorFormKeepsClientSchema` (`results_test.go`)
    проверяет round-trip всего набора клиентских ключей сразу (`groups`,
    `group_id`, `timer`, `visibility`, `default`, `suggestions`). До него
    round-trip формы не проверялся вовсе — при том, что ломался дважды.
  - Сворачивание блока во 2-й и последующих сериях и подстановку значений
    предыдущей серии делает мобильный клиент при вводе (дизайн:
    `docs/superpowers/specs/2026-09-11-sbe-lims-operator-form-groups-design.md`),
    поэтому в БД у каждой серии остаётся полный набор значений, а протокол,
    динамические таблицы, формулы и классификация не менялись.
  - `go build`/`go vet`/`go test -count=1 ./...` — чисто. Версия плагина
    `sbe-lims` поднята (1.0.7) — менялся и сам плагин, не только сервис.
  - **Задеплоено в тот же день**: контейнер пересобран, health `{"status":"ok"}`,
    в бинаре присутствуют обе новые метки схемы (`groups,omitempty`,
    `group_id,omitempty`) — это и есть подтверждение, что запущен новый код, а не
    надпись в логе сборки.

- **2026-09-09 — права переехали в auth-service.** Глобальная роль в приложении и
  общий доступ больше не читаются из локальных `lab_permissions`/
  `lab_common_access` — источник правды один на всю систему (дизайн:
  `docs/superpowers/specs/2026-09-09-central-permissions-design.md`, подробности —
  `sbe-core/auth-service/AGENTS.md`, запись 2026-09-09). Причина: семь копий
  одного кода прав разошлись, и в Фотобанке настройка «Нет общего доступа»
  молча перестала работать.
  - Новый `authperms.go` — **файл идентичен во всех plugin-сервисах** (их список — в корневом AGENTS.md,
    число здесь намеренно не дублируется), правки
    только сразу во всех (тест — в `photo-service/authperms_test.go`). Держит
    карту «email → роль» приложения, полученную из auth-service, со свежестью
    30 секунд; если auth недоступен, работает по последней известной карте и
    пишет предупреждение — падение auth не должно гасить все плагины разом.
  - Запись прав ушла прокси в auth-service (`setPermissionUpstream`/
    `setCommonAccessUpstream`); право проверяет он же. Внешние
    `/api/lab/permissions*` остались прежними — плагин и веб-портал не
    менялись.
  - Дефолт, когда общий доступ приложения не настроен вовсе: доступа нет.
  - Локальные таблицы прав оставлены нетронутыми как путь отката; данные
    перенесены в центральную таблицу с проверкой количеств.
...
- **2026-09-07 — новый эндпоинт `GET /api/lab/requests/search`: свободный поиск заявок
  ...
  (прошёл полный цикл brainstorming-скила: вопросы → варианты архитектуры → секции
  дизайна → одобрение пользователем). Жалоба пользователя: «в ЛИМС у нас десятки
  атрибутов, но поиск только по нескольким доступен». Разбор показал: `GET
  /api/lab/requests` не принимал вообще никаких параметров поиска, а материал/
  изготовитель/характеристики/наблюдения физически недостижимы клиенту — лежат в
  трёх разных JSONB-блобах (`objects.characteristics`, `measurement_results.values`,
  `aggregated_results.result_data`) с разным набором ключей на каждый метод
  испытания, без общего словаря атрибутов между методами.
  - **Только агент** (LogicTEAM.007, тул `get_lims_requests` с новым параметром
    `query`) — не UI плагинов ЛИМС/«Заявки на испытания» (по решению пользователя,
    осознанно вне объёма этой итерации).
  - **Архитектура** (`search.go`, новый файл): сервер на каждый запрос поиска сам
    "расплющивает" нужные таблицы в память — `loadVisibleRequests` (та же видимость,
    что у обычного списка, новой дыры в правах нет) + батч-запросы по одному на
    таблицу (`projects`/`inventors`/`objects`/`measurement_results`/
    `aggregated_results`, НЕ N+1 на заявку). На каждую заявку строится список полей
    (Название/Описание/Заказчик/Испытатель/Номера/Объект/Характеристики объекта/
    Результаты измерений/Итоговые результаты); JSONB-блобы разворачиваются в
    `ключ: значение`. Осознанно НЕ материализованный Postgres FTS-индекс (как в
    `photo-service`, `to_tsvector('russian')`) — при текущем объёме данных (сотни/
    низкие тысячи заявок) это была бы избыточная работа; формат ответа спроектирован
    так, чтобы позже заменить внутреннюю реализацию на FTS без слома контракта.
  - **Поиск**: запрос разбивается на значимые слова (короче 2 символов и частые
    предлоги отбрасываются), сравнение — регистронезависимое + транслитерация
    кириллица↔латиница в обе стороны (тот же приём, что уже чинил баг ПИР/PIR для
    писем/документов). Заявка совпадает, если КАЖДОЕ слово запроса нашлось хотя бы
    в одном поле (слова могут быть в разных полях — не точная фраза).
  - **`matched_in`** в ответе — для каждой заявки список `{field, snippet}`: где
    именно нашлось совпадение. Прямое требование пользователя во время брейнсторма —
    агент должен честно объяснять находку, а не выдумывать контекст.
  - Дополнительные фильтры `lab`/`status`/`compliance`/`date_from`/`date_to` — те же,
    что уже были у обычного списка заявок, применяются на сервере ДО поиска (можно
    сочетать: «прочность алюминия в лаборатории пожарных испытаний за март»).
  - Роут `GET /api/lab/requests/search` зарегистрирован ДО `GET /api/lab/requests/
    {id}` — конфликта нет, Go 1.22+ ServeMux сам отдаёт приоритет буквальному
    сегменту перед `{wildcard}`.
  - Юнит-тесты (`search_test.go`) на чистые функции `buildSearchFields`/
    `matchSearchWords`/`searchWords` — без БД: совпадение слов в разных полях,
    обязательность ВСЕХ слов запроса (не OR), транслитерация, поиск в
    `measurement_results.values`, отсечение предлогов, пустой запрос.
  - **Не делаем в этой итерации (сознательно)**: точные фасетные фильтры по
    конкретному id атрибута метода (диапазоны вроде «прочность ≥ 50») — атрибуты не
    стандартизированы между методами, это отдельная более крупная задача.
  - `go build`/`go vet`/`go test ./...` — чисто. Задеплоено на прод (`docker compose
    build lab` + `up -d lab`), health OK, роут отвечает 401 без авторизации
    (зарегистрирован).
  - Версия плагина `sbe-lims` НЕ поднимается — изменения только в
    `lab-service/` (ветка `backend`), сам плагин (TS/JS) не менялся.

- **2026-09-04 — фикс `handleSetTargetIndicator`: `jsonb_set` не создавал промежуточный ключ `target_indicators`.**
  Живой инцидент при массовой простановке целевых показателей по запросу
  пользователя (заявки проектов PVC MEMBRANE/PVC SAMPLES без заданного
  показателя — 292 ГГ/161 ГВ/42 РП). Первая же тестовая запись (`POST
  /requests/{id}/target-indicator`) вернула `{"ok":true}`, но значение не
  сохранилось. Причина — Postgres `jsonb_set(characteristics, ARRAY
  ['target_indicators', method_id], value, true)`: `create_missing=true`
  достраивает только ПОСЛЕДНИЙ уровень пути, а не промежуточные контейнеры —
  если `characteristics` вообще не содержал ключа `target_indicators`
  (486 из 502 объектов этих двух проектов), вызов молча возвращал документ
  без изменений. Обнаружено сразу, до запуска на всём массиве — успели
  проверить на одной заявке (939) до полного прогона.
  - **Фикс**: `jsonb_set` теперь одним уровнем прямо по `characteristics`
    (путь `{target_indicators}`, гарантированно существующий верхний
    объект), значение — уже домёрженный в SQL объект `coalesce(
    characteristics->'target_indicators', '{}'::jsonb) || jsonb_build_object
    (method_id, indicator)`. Дочитал по PostgreSQL-докам и перепроверил
    `SELECT`-дозапуском на реальном объекте (939) до деплоя.
  - Задеплоено на VDS, полный прогон повторён (идемпотентно — уже верно
    записанные 180 заявок из первого захода вернули 409, остальные 315
    записались и пересчитались). Итог сверен сквозным запросом: 0 пустых
    из 502 заявок, значения точно совпадают с запрошенными (кроме одной
    заявки, у которой целевой показатель уже был задан вручную раньше —
    корректно не тронута гвардом «только если не задан»).
  - `go build ./...`/`go vet ./...`/`go test ./...` — чисто.

- **2026-09-04 — `POST /requests/export-summary.xlsx` — сводный Excel-экспорт по нескольким заявкам.**
  Прямой запрос пользователя: при выделении нескольких заявок в веб-версии
  (механизм выделения уже был — под пакетную печать QR, `sbe-web/AGENTS.md`)
  скачать сводную таблицу с агрегированными результатами; если выбраны
  заявки нескольких методов — каждый метод на отдельном листе.
  - Новый файл `export_summary.go`, новый роут в `main.go` (сразу после
    одиночного `GET /requests/{id}/export.xlsx`), гейт `requirePerm("viewer")`
    + инлайновая проверка видимости `requireLabRead` на каждый id из тела
    запроса — недоступные заявки молча исключаются из выборки (тот же
    принцип, что и у общего списка видимых заявок — не палим вызывающему
    сам факт существования чужого id), 400 только если ВСЕ id оказались
    недоступны.
  - Загрузчики `loadMethodConfig`/`loadAggregatedRow`/`loadStatsRow` — те же
    самые, что уже использует одиночный `handleExportExcel` (`export.go`),
    просто в цикле по группе заявок одного метода вместо одной заявки.
    Идентификационные колонки (номер/внешний ID/объект/заказчик/дата)
    батчатся одним `id = ANY($1)` запросом на весь список (тот же паттерн,
    что `enrichResultCompliance` уже использует) — без N+1 по ним; сами
    agg/stats всё ещё грузятся по 2 запроса на заявку (N+1) — как и у
    одиночного экспорта, отдельного батч-загрузчика для них в кодовой базе
    не было, оставлено на будущее, если станет узким местом на больших
    выборках.
  - Имя листа = код метода (санитизация под лимиты Excel — без `: \ / ? * [ ]`,
    ≤31 символ, дедуп при коллизии через суффикс « (2)», « (3)» ...).
    Колонки результатов — объединение (union) ключей agg+stats по ВСЕМ
    заявкам группы, сортировка по алфавиту, подпись — то же имя атрибута
    метода, что уже резолвит одиночный экспорт (fallback на сырой ключ).
  - Плагин `sbe-lims` этой правкой не затронут (изменения только в
    `lab-service`) — версия не поднималась по правилу «бэк без изменений
    плагина не бампает версию».
  - `go build ./...`/`go vet ./...`/`go test ./...` — чисто.

- **2026-09-04 — `POST /requests/{id}/target-indicator` (владелец заявки); 4-й вид «Справка».**
  Две задачи пользователя.
  1. **`POST /api/lab/requests/{id}/target-indicator`** (`requests.go`,
     `handleSetTargetIndicator`) — узкий эндпоинт, отдельный от общего
     `PATCH /objects/{id}` (тот — editor+, полная замена `characteristics`,
     непригоден для точечной правки владельцем). Гейт: `email == owner_email`
     заявки ИЛИ `requireLabAccess` (сотрудник лабы/admin+). Валидирует
     присланный показатель по `determinable_indicators` метода — 400 при
     несовпадении. Гвард «только если ещё не задан» — 409
     `"целевой показатель уже задан — правка через справочник объектов"`,
     если `characteristics.target_indicators[method_id]` уже непусто (эта
     узкая дверь не даёт менять уже существующее значение — только закрывает
     пробел). Точечная запись через `jsonb_set(..., '{target_indicators,
     <method_id>}', ..., true)` — не трогает остальные поля characteristics
     (в отличие от общего PATCH). После успешной записи — вызывает
     `recalcRequestMethod` (та же функция, что использует существующая кнопка
     «Пересчитать» в плагине); ошибку пересчёта не превращает в 500 (данные
     уже сохранены) — возвращает `{"ok": true, "warning": "..."}`.
  2. **4-й вид вывода «Справка»** (`ShowInHelp`, `results.go`;
     `showInKind`/`filterBlocksForKind`/`templateKindFromQuery`/`docTitle`,
     `protocol.go`) — `template=help` на `POST /requests/{id}/protocol`.
     Появился, т.к. подсказка веб-портала над «Не соответствует» раньше
     ошибочно переиспользовала `ShowInExcerpt` (другое по смыслу назначение —
     «выписка»); теперь у неё свой независимый флаг, конфигуратор метода
     (`sbe-lims`) получил 4-й чекбокс.
  - **Задеплоено на VDS** (`docker compose up -d --build lab`, health OK).
    **`backend` НЕ перенесён в `main`** в этой сессии — по прямому решению
    пользователя (главная ветка отдаёт то, что было до этой правки; версия
    плагина — 1.0.4b, суффикс до отдельной команды на перенос).
  - `go build ./...`/`go vet ./...`/`go test ./...` — чисто. Живое
    CDP-тестирование (см. `sbe-web/AGENTS.md`) прогнало сценарий целиком:
    форма → выбор показателя → POST → пересчёт → подсказка «Справка» на
    обновлённом «Не соответствует» — 0 ошибок консоли.
...
- **2026-09-04 — Настройки лаборатории открыты для lab_admin; оповещения о поверке стали per-lab.**
  ...
  - **Живой инцидент при первом деплое (2026-09-04)**: предположение
    «таблица `equipment_notify_settings` ещё не деплоилась» (со ссылкой на
    более раннюю запись в этом файле от 2026-09-03) оказалось неверным для
    боевой БД — таблица со старой схемой (`id INT PRIMARY KEY DEFAULT 1`,
    одна строка) уже была на VDS, задеплоена транзитом при переносе v1.0.1
    из `backend` в `main` (запись «main синхронизирован», см. `sbe-lims/
    AGENTS.md`). `CREATE TABLE IF NOT EXISTS` с новой схемой (`lab_id`)
    поэтому был no-op — колонка `lab_id` не появлялась, контейнер `lab`
    падал в рестарт-цикл (`column "lab_id" of relation
    "equipment_notify_settings" does not exist`). Исправлено: явный `DO $$`
    guard в `main.go` — если есть колонка `id` и нет `lab_id`, переименовать
    и перенести строку `id=1` в `lab_id=0`; идемпотентно (на свежей или уже
    мигрированной базе колонки `id` нет — блок молча пропускается).
    Пересобрано и передеплоено успешно вторым заходом, health `{"status":"ok"}`,
    существующая строка настроек (`enabled=false, days='30,14,7'`) сохранена
    под `lab_id=0`. Урок: не полагаться на текстовое описание в AGENTS.md о
    статусе деплоя без прямой проверки живой схемы (`\d table`) перед тем,
    как менять `CREATE TABLE` вместо аккуратной ALTER-миграции.
...
- **2026-09-03 — «Просмотр от лица роли» (веб) + три фикса видимости заказчика/группы.**
  - **«Просмотр от лица роли»** (по прямому запросу пользователя): заголовок
    ...
    только `channel=web`, только на роль не выше уже клэмпнутой (`admin` —
    веб никогда не даёт `superadmin`). Реализовано ВНУТРИ `effectiveRole()`
    (`jwt.go`) — единая точка, от которой зависят и `requirePerm`-гейт, и вся
    видимость данных внутри обработчиков, поэтому симуляция ограничивает
    реально всё, а не только вёрстку. `rawRole()` — реальная роль без
    подмены, для `handleMyPermission` (`real_role` в ответе — иначе
    суперадмин, включивший просмотр как viewer, терял бы сам переключатель).
    E2E на реальном суперадмине: 581 видимая заявка как admin → 18 как
    viewer (ровно те, что реально принадлежат этому email).
  - **`requireMinRoleForWebChannel`**: смена статуса заявки
    (`/requests/{id}/status`, `/kanban-move`) для editor через веб запрещена
    — те же роуты через Obsidian-плагин (`channel=plugin`) работают как
    раньше. Кнопки статуса в вебе остаются видимыми для editor, но
    неактивными (UI), сервер — финальный страж.
  - **`requireLabRead` (`lims_refs.go`)**: раньше проверял ТОЛЬКО членство в
    `lab_members` (отдельный список штатных сотрудников) — заказчик
    (`owner_email`) и участник группы заявки видели её в списке, но получали
    403 при попытке открыть результаты/протокол/экспорт. Добавлена та же
    видимость, что уже используется для списка заявок (`requestVisible`).
    Заодно `protocol`/`export.xlsx` понижены с `editor` до `viewer` на
    роуте (main.go) — viewer раньше не мог даже дойти до `requireLabRead`.
  - **`handleListLabs` (`references.go`)**: та же болезнь — список
    лабораторий для не-admin строился только из `lab_members`, поэтому
    название лаборатории СВОЕЙ видимой заявки не резолвилось на клиенте
    (показывался голый `lab_id`). Добавлена видимость по владению/группе
    видимых заявок.
  - Все четыре фикса проверены E2E на реальном пользователе (создание
    тестового устройства/ключа `channel=web`, удалены после проверки).
...
- **2026-08-29 — исходящая почта переведена на аутентифицированную отправку
  ...
  (`noreply@epyur.fvds.ru`, изначальная схема WP2, `host.docker.internal:25`)
  в ряде случаев попадали в спам. `sendMailWithAttachment` (`outbound_email.go`)
  теперь аутентифицируется через `smtp.PlainAuth` теми же `LAB_MAIL_LOGIN`/
  `LAB_MAIL_PASSWORD`, что уже используются для ЧТЕНИЯ почты
  (`email_ingest.go`) — реальный провайдер (Yandex) с валидным сертификатом/
  SPF/DKIM вместо локального релея. Дефолты (если `LAB_SMTP_HOST`/`_PORT` не
  заданы) — `smtp.yandex.ru:587` (STARTTLS); `from` по умолчанию — тот же
  логин, если `LAB_SMTP_FROM` не задан.
  - **docker-compose.yml** (сервис `lab`): `LAB_SMTP_HOST: host.docker.internal`
    → `smtp.yandex.ru`, `LAB_SMTP_PORT: "25"` → `"587"`, `LAB_SMTP_FROM:
    ${SMTP_FROM}` → `${LAB_MAIL_LOGIN:-}` (тот же логин, не отдельная
    переменная), `LAB_SMTP_SKIP_VERIFY: "1"` — удалена строка целиком
    (Yandex — настоящий сертификат, обходить проверку больше незачем).
    `extra_hosts: host.docker.internal:host-gateway` оставлен — не мешает,
    убирать не входило в объём этой правки.
  - `go build`/`go vet`/`go test` — чисто. Задеплоено на VDS (`scp` +
    `docker compose build lab` + `up -d lab`), health ok. **Живой E2E
    транспорта**: аутентифицированная SMTP-сессия к `smtp.yandex.ru:587`
    (STARTTLS + AUTH PLAIN теми же `$LAB_MAIL_LOGIN`/`$LAB_MAIL_PASSWORD`,
    что реально в контейнере) — `curl --url smtp://... --ssl-reqd --mail-from
    ... --mail-rcpt ... --user ...` изнутри контейнера `lab`, ответ сервера
    `250` (принято), `curl` exit 0 — тот же сетевой путь/порт/креды, что
    использует Go-код. Не проверено этой сессией: реальная отправка ЧЕРЕЗ
    сам Go-код (`handleSendRequestEmail`/авто-триггеры) и то, что письмо
    больше не попадает в спам у получателя — пользователю стоит нажать «✉
    Отправить письмо» на реальной/тестовой заявке и проверить входящие.

- **2026-08-29 — уведомление в LPITrack при переходе заявки в «В работе»;
  ручной адрес служебных писем per-лаба; фикс темы служебного письма.**
  Три связанных живых находки/запроса в одной сессии по итогам первого
  реального E2E исходящей почты (заявка 428/2026-ГВ):
  1. **Не баг**: письмо после завершения не ушло, т.к. `labs.auto_send_email`
     было выключено у лабы ЛПИ (`sent_emails` — 0 строк вообще, ни одной
     попытки за всю историю; логи `lab` за 6ч — ни одного упоминания
     outbound/email). Подтверждено пользователем: включил галочку в
     настройках лабы — письма пошли.
  2. **Новый триггер `triggerProcessingEmail`** (`outbound_email.go`,
     вызывается из `handleSetRequestStatus`/`handleKanbanMove` — тот же
     паттерн, что `triggerCompletionEmails`, на переход в `processing`
     вместо `completed`). Только в LPITrack (заказчику на этом этапе
     сообщать нечего), с email назначенного испытателя (`assigned_to`
     — в системе нет отдельного «имени», только email, тот же принцип
     идентификации, что и везде в проекте). Тот же переключатель
     `auto_send_email` (не отдельный флаг), тот же дедуп через
     `sent_emails` (новый `recipient_type="lpitrack_processing"`) — повторный
     переход в processing (напр. туда-обратно через received) не шлёт письмо
     ещё раз. Чистая функция `shouldAutoSendProcessing` вынесена для
     юнит-теста (по образцу `shouldAutoSend`).
  3. **`labs.service_email`** (новая колонка, ручной адрес служебных писем
     этой лабы) — живая жалоба: письмо ушло не на тот адрес (глобальный
     `LAB_LPITRACK_EMAIL` из `.env` не годился именно этой лабе). Новая
     функция `serviceEmailFor(labID)` — приоритет `labs.service_email`, пусто
     — фоллбэк на env-переменную (обратная совместимость). Использована и в
     `sendCompletionEmails` (дубль в LPITrack при completed), и в
     `triggerProcessingEmail` — единая точка резолва адреса. UI — новое поле
     в форме лабы (`lims-view.ts`, `service_email` + `Lab.service_email` +
     `syncService.updateLab`), рядом с чекбоксом `auto_send_email`. Адрес
     заказчика этого поля НЕ касается — как и раньше, берётся из
     `requests.owner_email`.
  4. **Тема служебного письма** — была `external_id=775`, не совпадала с тем,
     что пользователь видит в поле «Внешний идентификатор» (полный ключ
     легаси-трекера `LPIZAYAVKINAPRO-775`, восстанавливается на клиенте
     константой `EXTERNAL_ID_PREFIX`). Новая функция `legacyExternalKey`
     (обратная операция к `strings.TrimPrefix(ID, "LPIZAYAVKINAPRO-")` в
     `email_ingest.go`) — тема обоих служебных писем (completion-дубль и
     processing-уведомление) теперь `LPIZAYAVKINAPRO-<external_id>`. Тело
     письма не менялось (там `external_id=` остаётся как внутренняя
     техническая деталь текста, не тема).
  - `go build`/`go vet`/`go test`/`gofmt -l` (только реально изменённые
    файлы — репозиторий в целом содержит давние, не связанные с этой правкой
    CRLF/отступ-нюансы gofmt в файлах, которых эта сессия не касалась) —
    чисто. Задеплоено на VDS (`scp` + `docker compose build lab` + `up -d
    lab`), миграция `service_email` прошла, health ok.
  - **Не проверено живым E2E до конца** (нет времени в рамках этой сессии
    прогнать полный цикл received→processing→completed с реальным назначенным
    испытателем и новым `service_email`) — пользователю стоит: (а) указать
    правильный адрес в новом поле «Адрес служебных писем» карточки лабы,
    (б) перевести тестовую заявку в «В работе» и подтвердить, что письмо
    пришло на верный адрес с темой `LPIZAYAVKINAPRO-<N>` и именем испытателя
    в теле.

- **2026-08-29 — сверка статусов/дат заявок с внешней таблицей
  LPIZAYAVKINAPRO (Jira-выгрузка).** Прямой запрос пользователя: сверить все
  заявки с `C:\Users\adm\Downloads\LPIZAYAVKINAPRO_29-08.xlsx` (столбцы «Ключ»
  = external_id без префикса `LPIZAYAVKINAPRO-`, «Резолюция», «Дата начала»,
  «Дата завершения» — по одной строке на КАЖДЫЙ метод заявки, не на заявку
  целиком, т.к. в старой системе каждый метод — отдельная задача-тикет).
  Правило маппинга статуса: `Резолюция == "Решен"` → `status='completed'`,
  иначе (включая пустую резолюцию) → `status='new'` (единственный второй
  статус, реально используемый в БД на момент сверки — `processing` не
  занят ни одной заявкой). Даты — `created_at`/`completed_at` `requests`
  (единственные системные timestamptz-поля уровня заявки; НЕ путать с
  `exp_date`/`report_date`/`samples_in_date` — те текстовые, per-серия,
  заполняются испытателем при вводе результата).
  - Таблица: 757 строк, все уникальные external_id, 0 дублей/пустых ключей.
    Все 564 заявки нашей БД с непустым `external_id` нашли соответствие в
    таблице — **заявок «у нас есть номер, а в таблице нет» не найдено**.
  - Применено одной транзакцией (`BEGIN`/temp-таблица со всеми 757 строками
    таблицы/`UPDATE ... FROM`/`COMMIT`) после бэкапа
    (`pg_dump -t requests --data-only` →
    `/root/lab_requests_backup_before_lpiz_sync_20260829.sql` на VDS,
    не в git): **72 смены статуса** (65 `new→completed` — заявки реально были
    решены в старой системе, но статус в нашей БД не обновлялся; 7
    `completed→new` — обратный случай, у нас помечены завершёнными, а по
    таблице резолюции нет) и **558 из 564 синхронизаций дат** (в основном
    `completed_at` — раньше почти нигде не заполнялся, т.к. поле появилось
    позже исторических данных и не бэкфилилось). Один нюанс источника:
    external_id=652 — резолюция пустая (значит `status='new'`), но «Дата
    завершения» в таблице всё же заполнена (похоже на переоткрытую после
    закрытия задачу, тег `otkpvc` в исходных данных) — синхронизировано
    буквально как в таблице (`completed_at` установлен, `status='new'`), без
    доп. эвристик поверх явного запроса «синхронизировать... с указанной
    таблицей». 1 строка таблицы без «Дата начала» — `created_at`
    соответствующей заявки не тронут (COALESCE к текущему значению).
  - Итог по статусам: было 469 completed / 95 new → стало 527 completed / 37
    new (469+65-7=527, 95-65+7=37 — сходится). Точечно проверено на
    external_id 665/664/663/734/652 после применения — все поля совпадают с
    ожидаемым.
  - Только данные (UPDATE), код не менялся — задеплоенного кода это не
    касается.

- **2026-08-29 — фикс калибровочной кривой метода РП (заявка 286/2026-РП);
  массовый пересчёт всех заявок (`recalc-all`).** Живая жалоба пользователя:
  формула «Критическая плотность теплового потока» (`agg_flam_flow_density`,
  метод РП), предложенная ИИ-помощником формул, не считалась. Найдены и
  исправлены ДВЕ независимые причины:
  1. **Неверные имена параметров в самой формуле.** Формула была
     `interpolate(average_damage_length, xs, ys)` — но `xs`/`ys` нигде не
     существуют: точки калибровочной кривой подставляются в
     `FormulaEnv.Params` под именами `{calibration_attr_id}_xs`/`_ys`
     (`injectCalibrationCurves`, `calibration_curve.go`) — для метода РП это
     `heat_flow_curve_xs`/`heat_flow_curve_ys` (id калибровочного атрибута —
     `heat_flow_curve`). ИИ-помощник формул (`llm-assist.service.ts
     suggestFormula`) в промпт получает только обычные `input_parameters`
     метода — список `calibration_attributes` ему НЕ передаётся, поэтому он
     физически не мог знать реальное имя и придумал общее `xs`/`ys`. Это
     ограничение ИИ-помощника нигде раньше не было задокументировано —
     занесено сюда как известный пробел (нужно расширить промпт списком
     калибровочных атрибутов метода, чтобы ассистент писал имя
     `{id}_xs`/`{id}_ys` сам). Формула в БД (`methods.id=3`,
     `input_parameters`+`formulas`, атрибут `agg_flam_flow_density`) исправлена
     прямым SQL UPDATE на правильные имена.
  2. **Реальный баг бэкенда, независимый от (1).** `injectCalibrationCurves`
     вызывался ТОЛЬКО из `applyFormulas` (уровень серии) — из
     `applyAggregatedFormulas` (уровень заявки+метода) не вызывался вовсе. А
     КППТП сконфигурирован именно как `aggregated`-атрибут (считается из
     `average_damage_length`, который сам — среднее по сериям). Значит даже
     при правильных именах параметров формула бы не сработала — калибровочная
     кривая просто не подставлялась в `env` на агрегированном уровне. Фикс:
     `results.go` `applyAggregatedFormulas` — добавлен вызов
     `s.injectCalibrationCurves(ctx, requestID, methodID, 0, cfg, env)` перед
     `evalAggregatedFormulas` (equipmentID=0 → тот же best-effort
     auto-resolve через `resolveSingleMainEquipment`, что и на уровне серии,
     когда конкретная запись явно не указывает оборудование).
  3. **Новая CLI-команда `recalc-all`** (`recalc_all.go`, по прямому запросу
     «пересчитай все заявки») — проходит по каждой паре (request_id,
     method_id) из `measurement_results` и повторяет тот же порядок действий,
     что `handleCalculateSeries` для одной заявки. Ради этого
     `handleCalculateSeries` отрефакторен — его прежний инлайновый цикл
     вынесен в переиспользуемый `(s *Server) recalcRequestMethod(ctx,
     requestID, methodID) error`, чтобы HTTP-хендлер и массовый CLI-проход не
     дублировали одну и ту же логику. Ошибка формулы отличается от прочих
     ошибок через существующий `*formulaApplyError` (был объявлен в
     `results.go` — переиспользован, не продублирован).
  - **Проверено на живых данных**: `average_damage_length=310` →
    `interpolate` между калибровочными точками (300,7) и (400,5) даёт `6.8` —
    посчитано вручную, совпадает с результатом. `flame_spread_group="РП3"`,
    `target_flame_spread_group_compliance="Не соответствует"` — тоже
    досчитались (раньше отсутствовали, т.к. зависели от `agg_flam_flow_density`).
    `./mailers-lab recalc-all` прогнан по ВСЕЙ БД — 453 пары (заявка, метод),
    453 успешно/0 с ошибкой (ошибка формулы у ОДНОЙ пары не блокирует
    остальные — только логируется и пропускается). Все 19 заявок по методу РП
    в системе теперь имеют посчитанный `agg_flam_flow_density` (раньше — ни
    одной).
  - **Побочно замечено при массовом пересчёте (не чинилось в рамках этой
    записи)**: 268 заявок по методу ГГ логируют пропуск classification-правила
    `combustibility_group` из-за отсутствия `smoke_temp_combustibility_group`
    — то есть у них не считается ИТОГОВАЯ группа горючести целиком, не только
    температура дыма. Требует отдельного разбора (возможно, тот же класс
    проблемы — имя параметра/отсутствующая agg-формула, возможно —
    легитимно отсутствующие данные): не входило в объём этой жалобы (РП/286),
    решение по объёму — за пользователем.
  - `go build`/`go vet`/`go test` — чисто. Задеплоено на VDS (`scp` +
    `docker compose build lab` + `up -d lab`), health ok. Формула метода РП
    исправлена прямым SQL UPDATE на проде (по явному разрешению
    пользователя — деплой/SQL иначе блокировались классификатором
    разрешений auto-режима).
...
## Зеркалирование фото испытаний в Фотобанк — довязка (2026-09-04)

`photobank_client.go` + юнит-тесты (`photobank_client_test.go`) существовали уже с
2026-09-01 (см. `docs/superpowers/specs/2026-09-01-sbe-lims-photobank-mirror-design.md`),
но никогда не вызывались из `saveResultSeries` — несмотря на чистые `go build`/`go vet`/
`go test`, функция была мёртвым кодом (не выполнялась ни разу). Обнаружено и довязано по
прямому запросу пользователя, который считал задачу уже закрытой.

- `saveResultSeries` (`results.go`) — сразу после `applyAggregatedFormulas`: диф
  `beforeValues`/`values` по 4 фото-полям (`changedPhotoFields`), на каждое новое/
  изменившееся значение — `go mirrorPhotoToPhotobank(context.WithoutCancel(ctx), ...)`,
  best-effort, не блокирует сохранение результата испытания.
- Легаси-импорт (`who=="legacy-import"`, см. `import_legacy_results.go`) явно исключён —
  по спеке зеркалируются только фото, загруженные ПОСЛЕ включения функции, не перенесённые
  исторические.
- Три фото с шаблонным названием «Заявка X, серия N, до/после испытания», уже видимые в
  Фотобанке (`Испытания/Лаборатория пожарных испытаний`) на момент этой сессии — НЕ
  результат работы этого кода (проверено: полный поиск по всей git-истории, `git log --all
  --pickaxe-regex -S"photobank"`, не находит ни одного коммита с вызовом в `results.go`);
  похоже, загружены вручную по той же конвенции именования, пока автоматизация не была
  подключена — не считать их подтверждением работоспособности автоматического пути.
- `go build ./...`/`go vet ./...`/`go test -count=1 ./...` — чисто. **Не задеплоено** —
  только локальная сборка на ветке `backend`. Живой E2E пока невозможен — испытаний с фото
  после включения функции ещё не было; проверять на первом же реальном сохранении фото
  серии после деплоя (папка `Испытания/<лаба>` в Фотобанке должна пополниться сама).

## Зеркалирование в Фотобанк было вызвано, но никогда не срабатывало для мобильных фото + защита photo_before/photo_after от затирания (2026-09-24)

Живая жалоба: испытатель на планшете (ЛИМС Мобайл) поймал «s3 error» при загрузке
фото. Разбор ошибки — внешний сбой (см. следующий раздел), но попутно нашёлся
настоящий баг в зеркалировании из раздела выше: вызов был на месте с 2026-09-04, но
**никогда не находил изменений** для обычных фото мобильного приложения.

- **Причина**: `photo_before`/`photo_after` — ОТДЕЛЬНЫЕ колонки `measurement_results`
  (не внутри `values`), это явно задокументировано в `email_ingest.go` («photo_before/
  photo_after ЗДЕСЬ не перечислены... applyResultPayload ОТДЕЛЬНО... кладёт их в
  измеренные photo_before/photo_after колонки»). Но `changedPhotoFields(beforeValues,
  values)` в `saveResultSeries` сравнивал именно `values` (карту атрибутов формы) — ключей
  `photo_before`/`photo_after` там нет НИ У ОДНОГО вызывающего кода, ни у мобильного
  апсерта, ни у почтового. Реально мог сработать только для `photo_before_test`/
  `photo_after_test` — другой, отдельной пары атрибутов формы, не того, что льёт пикер
  мобильного приложения.
- **Смежный, более серьёзный баг, найден в той же сессии**: `sbe-lims-mobile`
  (`mobile-lims-view.ts`) при КАЖДОЙ перерисовке формы серии (переключение на другую
  серию и обратно — `redrawForm`) инициализировал `photoBeforeUrl`/`photoAfterUrl`
  пустой строкой, не подставляя уже сохранённое фото из `existingSeries`. Сервер писал
  что пришло, безусловно (`photo_before = EXCLUDED.photo_before`). Итог: повторное
  сохранение уже сфотографированной серии (даже ради правки несвязанного поля) могло
  молча стереть снимок из протокола — тот же класс бага, что чинил `carryOverUnownedValues`
  2026-09-21 для обычных значений формы, просто эти две колонки лежат отдельно от `values`
  и тот перенос их не касается.
- **Фикс в `saveResultSeries`**: перед апсертом теперь читаются СТАРЫЕ `photo_before`/
  `photo_after` тем же запросом, что уже читал старые `values`; пустая строка от клиента
  больше не затирает сохранённое (то же «пусто — не трогать», что у `report_date` и т.п.).
  `changedPhotoFields` теперь сравнивает правильные старое/новое значения (собраны в
  одноразовые карты поверх `beforeValues`/`values`, сама функция не менялась — она общая
  и уже была покрыта тестами).
- Мобильный клиент (`sbe-lims-mobile` 0.1.19) — предзаполняет `photoBeforeUrl`/
  `photoAfterUrl` из `existingSeries` и показывает превью уже прикреплённого фото при
  открытии формы; это снимает практический повод для бага (испытатель больше не видит
  пустой пикер там, где фото уже есть), но именно данные защищает СЕРВЕРНЫЙ фикс выше.
- `go build`/`go vet`/`go test ./...` — чисто, регрессий в существующих тестах
  `changedPhotoFields` (`photobank_client_test.go`) нет.
- Дизайн зеркалирования (2026-09-01) и мобильного пикера этот фикс не меняет —
  только починка триггера и защита колонок от затирания.

## «s3 error» на самом деле — смена адреса S3-шлюза хостером, не сетевая авария (2026-09-24)

Тот же разбор, что выше: DNS для `s3.firstvds.ru` (через который сервис ходит в S3 по
rclone) с утра отдавал NXDOMAIN. Прямой запрос авторитетных NS `firstvds.ru`
(`ns1/ns2.ddos-guard.net`) подтвердил — не кэш и не наша конфигурация. По наводке
пользователя («в панели управления S3 определяется другой адрес») нашли: хостер завёл
новый адрес шлюза `firsts3.ru` (сертификат `*.firsts3.ru` выпущен ещё 25 августа —
за месяц до отключения старого имени) и **сегодня утром убрал DNS-запись старого
`s3.firstvds.ru`** без предупреждения. Проверено авторизованным rclone (боевые ключи) —
`firsts3.ru` отдаёт тот же бакет `sbe-doc`, те же файлы.

**Исправлено и уже задеплоено** (не через это репо — прямая правка `/opt/mailers/.env`
на сервере, не в git): `S3_ENDPOINT=https://s3.firstvds.ru` → `https://firsts3.ru`.
Эта переменная общая на пять сервисов (`lab`, `photo`, `documents`, `agent`, `kb`) —
пересозданы все пять контейнеров разом. Проверено end-to-end: `GET /api/lab/file-redirect`
на файле, который утром падал с DNS-ошибкой, теперь отдаёт 302 → `firsts3.ru` → 200,
верный `Content-Type`/размер. Резервная копия `.env` — `.env.bak-20260924-s3endpoint`
рядом на сервере.

**Вывод в общую копилку**: «остальные данные уходят, а фото — нет» с ошибкой S3 не
обязательно значит временный сбой сети — стоит явно проверять, не сменился ли у
хостера сам адрес шлюза (сравнить с тем, что показывает его панель управления), прежде
чем считать это внешней аварией, с которой можно только подождать.

## Буфер загрузки при недоступности S3 (2026-09-24)

По итогам разбора двух разделов выше — пользователь запросил защиту от будущих
подобных случаев: если S3 всё-таки станет недоступен, файл должен приниматься на
локальный диск сервера и дозаливаться в S3 сам, без участия человека и без потери
попытки загрузки. Прошли полный цикл брейншторминга, дизайн —
`docs/superpowers/specs/2026-09-24-s3-upload-buffer-design.md`, план —
`docs/superpowers/plans/2026-09-24-s3-upload-buffer-plan.md`. Тот же механизм внедрён
и в `photo-service` (см. его `AGENTS.md`) — по образцу уже работающего там
`pending_deletes` (`cleanup.go`).

- Новая таблица `pending_uploads` (`key`, `file_size`, `queued_at`, `attempts`,
  `last_error`) — миграция в `main.go`.
- `S3Store` (`s3.go`): `bufferDir`/`bufferMaxBytes` (env `UPLOAD_BUFFER_DIR`/
  `UPLOAD_BUFFER_MAX_BYTES`, по умолчанию 10 ГБ), `rcloneArgs` теперь принимает
  `context.Context` и вызывает `exec.CommandContext` с таймаутом 25 сек — раньше
  контекст в `exec.Command` не пробрасывался вовсе, зависшее (не мгновенно отказавшее)
  соединение держало бы HTTP-запрос неограниченно долго. 30-секундное «остывание»
  после неудачи — следующие `Put` этот срок сразу уходят в буфер, не дожидаясь
  повторного таймаута при затяжной аварии.
  - `Put`: лимит буфера проверяется ДО записи (сумма из `pending_uploads`), при
    превышении — `ErrBufferFull` (`handleUploadFile` отвечает 503 понятным текстом,
    не общим «s3 error»); при ошибке `rclone` — байты на диск + строка в очередь +
    толчок воркера, вызывающему коду — штатный успешный ответ с тем же ключом/URL.
  - `Get`: сначала `os.Stat` буфера, потом (если пусто) — S3 как раньше. Это же
    автоматически чинит `mirrorPhotoToPhotobank`/`fetchPhotoBytes` — отдельных правок
    не потребовалось.
- `upload_buffer.go` (новый файл) — фоновый воркер (`startUploadFlushWorker`, тикер
  2 мин + канал-«толчок» + проход при старте, тот же приём, что у `cleanup.go`
  photo-service), `GET /api/lab/buffer/status` (admin) — количество/суммарный
  размер/возраст самой старой записи/«застрявших» (после 50 неудач подряд файл и
  строка остаются на месте, просто выходят из автоматических попыток).
- `files.go` (`handleFileRedirect`) — если ключ ещё в буфере, отдаёт байты напрямую
  (200, `Content-Type` по расширению ключа), не пытаясь получить presigned-ссылку
  на объект, которого в S3 ещё нет.
- `go build`/`go vet`/`go test ./...` — чисто, `gofmt -l` пуст. Юнит-тесты на чистую
  логику (`upload_buffer_test.go`: `bufferPath`, «остывание»). Живой E2E на сервере
  не проводился (после фикса `S3_ENDPOINT` в этой же сессии реальной аварии для
  проверки уже не было) — стоит проверить после деплоя временной подменой
  `RCLONE_CONFIG` на нерабочий эндпоинт, как описано в спеке.
- **Bind-mount на хосте** (`/opt/mailers/lab-service/upload-buffer/`), переменные
  окружения и `docker-compose.yml` — в `sbe-core` (ветка `backend`), см. его AGENTS.md.
  Каталог создаётся деплой-скриптом (`scripts/lab_deploy.sh`), если его ещё нет.
- Версия плагина `sbe-lims` НЕ поднимается — правка только бэковая.

## Легенда графика — подрисуночная подпись под графиком, не список внутри поля (2026-09-24)

Живая жалоба: у метода ГГ несколько каналов датчика (4 термопары + средняя +
производная — 6 серий на одном графике), и легенда, введённая 2026-08-29 (см. раздел
выше, «Фикс: легенда графика с несколькими сериями») как список ВНУТРИ поля графика
(правый верхний угол), при таком числе серий перекрывала половину данных.

- `charts.go`: старый `drawLegendBox` убран целиком, вместо него — `layoutLegendRows`
  (жадный перенос элементов по ширине, чистая функция) + `legendCaptionHeight` +
  `drawLegendCaption` (рисует строки легенды центрированными ПОД графиком, после
  подписи оси X). Высота изображения (`totalH` в `renderChart`) теперь растёт под
  подпись — считается ДО создания `image.NewRGBA`, поэтому легенда никогда не
  обрезается и не накладывается на область данных, сколько бы строк ни потребовалось.
- Проверено визуально (временный PNG, локально) на 1-строчном и 2-строчном (длинные
  подписи) случаях — легенда чисто переносится, поле графика остаётся свободным.
  Закреплено регрессионными тестами в `charts_test.go`: `TestLayoutLegendRowsWrapsByWidth`,
  `TestLegendCaptionHeightEmptyIsZero`, `TestRenderChartLegendCaptionGrowsImageNotPlot`
  (декодирует настоящий PNG и сверяет, что высота выросла ровно на расчётную высоту
  подписи, а ширина не изменилась).
- `go build`/`go vet`/`go test ./...` — чисто, `gofmt -l` пуст.
- Правка касается ЛЮБОГО графика, построенного `renderChart` (не только метода ГГ) —
  функция общая на все методы лаборатории.
