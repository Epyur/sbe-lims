...
**⚠️ Не сделано (требует решения пользователя, не тронуто намеренно):**
массовый повторный разбор ОСТАЛЬНЫХ ~1862 (`unknown type`) + ~583
(`method2`) исторических писем — код исправлен и живьём проверен на единичных
примерах (`fetch-mail`), но у обычного воркера есть дедуп по `message_id`
(`email_ingest_processed`) — уже помеченные "error" письма НЕ подхватятся
автоматическим фоновым опросом, только точечно через `fetch-mail` (по одному
ID) или явной очисткой dedup-записей + повторным проходом воркера по всей
папке (риск — `Flam`/`Comb` содержат 1000+ писем каждая, на больших папках
уже ловили `imap: connection closed`, см. WP7 выше — массовый повтор нужно
делать аккуратно, не одним махом).
