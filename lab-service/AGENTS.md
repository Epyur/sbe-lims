...
- **2026-09-21 — ⚠️ реальная потеря данных: сохранение формы стёрло данные
  ...
  - **Правка:** `carryOverUnownedValues` — поля, которых нет во входящем наборе
    ...
    Тесты — `results_carryover_test.go`. **Задеплоено 2026-09-21**
    (`scripts/lab_deploy.sh`), наличие правки в бинаре проверено; восстановленные
    данные заявки 1578 перезапуск пережили. Не закоммичено.
