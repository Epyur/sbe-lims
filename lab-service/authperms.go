...
// ВНИМАНИЕ: этот файл ИДЕНТИЧЕН во всех plugin-сервисах — lab, photo, documents,
// contacts, ekn, mailer, agent и с 2026-09-10 kb, всего восемь. Правки — сразу
// во всех; тест живёт в photo-service (authperms_test.go). Общим go-модулем не
// сделан сознательно: у сервисов отдельные модули и отдельные Docker-контексты
// сборки, ради ста строк транспорта переделывать сборку восьми сервисов дороже
// (см. дизайн docs/superpowers/specs/2026-09-09-central-permissions-design.md,
// раздел «Компромисс, который остаётся»).
//
// Сверять копии между собой только БЕЗ учёта переводов строк. В репозитории
// файл хранится с LF, но на Windows при core.autocrlf=true git подставляет в
// рабочую копию CRLF везде, где у сервиса нет .gitattributes с `*.go text
// eol=lf` (такой есть только у kb-service). Поэтому побайтовое сравнение (md5)
// рабочих копий показывает ложное расхождение при одинаковом содержимом, а
// `gofmt -l` помечает файлы тех сервисов, куда CRLF уже попал (на 2026-09-11 —
// photo-service и, шире, большая часть Go-файлов agent/documents/lab/mailer).
//
