...
//
// Слов больше, чем подписей в интерфейсе, и так и задумано. «Обычный»,
// «Критичный», «Блокер» приходят из писем легаси-трекера — внешней системы,
// шаблоны которой мы не меняем, поэтому убрать их нельзя. «Средний»,
// «Критический», «Блокирующий» добавлены 2026-09-21 вместе со стандартизацией
// подписей: письмо могут составить и руками, глядя на наш интерфейс.
func mapEmailPriority(raw string) string {
	switch strings.TrimSpace(raw) {
	case "", "Обычный", "Средний":
		return "normal"
	case "Критичный", "Критический":
		return "critical"
	case "Блокер", "Блокирующий":
		return "blocker"
	default:
