...
// измерения температуры") — своя видимость в каждом из фиксированных видов
// вывода (2026-08-22: "ровно 3, простые галочки", по решению пользователя —
// НЕ расширяемый список именованных шаблонов; 2026-09-04: добавлен 4-й,
// "Справка", см. ShowInHelp — тот же принцип, отдельная именованная галочка,
// не общий расширяемый список) + опциональный привязанный график. Title —
// только для списка блоков в редакторе, не печатается в документе (в отличие
// от заголовков RichNode "heading", которые пользователь сам вставляет как
// содержимое, если нужен видимый заголовок раздела).
type DocumentBlock struct {
	...
	// ShowInHelp — 4-й вид вывода, "Справка" (2026-09-04, по прямому запросу
	// пользователя): содержимое всплывающей подсказки над "Не соответствует" в
	// веб-портале. Раньше подсказка ошибочно переиспользовала ShowInExcerpt
	// (другое по смыслу назначение — "выписка") — теперь у неё свой независимый
	// флаг, админ метода может выбрать разное содержимое для выписки и подсказки.
	ShowInHelp bool `json:"show_in_help"`
}

// MethodPresentation — methods.presentation: блоки форматированного текста.
// Фиксированные виды вывода читают один и тот же набор блоков, отличаясь
// только фильтром ShowInUI/ShowInExcerpt/ShowInProtocol/ShowInHelp на блоке —
// состав "выписки"/"справки" не зафиксирован программно, админ метода решает
// сам, что туда включить.
