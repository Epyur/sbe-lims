...
| POST | `/lab-members` | admin | `{lab_id,email,role}` → `{ok}` |
...
Справочники испытателей/оборудования — editor+; методы-конфиги и мутации `lab-members`
(POST/DELETE) — admin+; чтение `lab-members` с `?lab_id=` — любой участник этой лабы
(2026-08-24, см. выше). Kanban-доска «Очередь лаборатории»: «руководитель лабы» в
терминах задачи = глобальная роль admin/superadmin (не лабораторный `lab_admin` —
свободное перетаскивание/назначение carточек привязано к УЖЕ проверенной клиентом роли
`canAdmin`, не к отдельной lab-scoped проверке); «испытатель» = `lab_operator`/`lab_admin`
ИМЕННО этой лабы (`kanban-move` проверяет через `labMemberRole`).
...
- Вьюха «LogicLAB.ЛИМС» (2026-08-21, было «Лабораторная информационная менеджмент
  ...
  - **Контент-карточка**: заголовок раздела + подзаголовок; **все 8 пунктов дерева
    ...
    - «Сотрудники» — список (по текущей лабе) + добавление/удаление, admin (раздел
      скрыт для не-admin — читает `GET /lab-members` без `?lab_id=`, полный список,
      admin-only на сервере; Kanban-доска/деталь заявки читают ростер СВОЕЙ лабы через
      `?lab_id=`, доступно любому участнику, 2026-08-24 — см. «Роли» выше).
...
- **Реализовано 2026-08-24**: точная клиентская проверка `lab_operator`/`lab_admin`
  ...
  Область действия — ТОЛЬКО статус/назначение заявки (см. «Очередь лаборатории» выше);
  `canEdit` (результаты/расчёт) по-прежнему разрешает всё любой app-роли `!== ''`,
  сервер всё равно валидирует по `requireLabAccess`/`requireLabRead`.
- **Запланировано**: делегированные полномочия `lab_admin` внутри своей лабы за
  пределами Kanban-доски (не реализовано — см. «Роли»).
