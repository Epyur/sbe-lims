...
| POST | `/requests/{id}/status` | editor+ | `{status}`: `new`/`received`/`processing`/`completed` → `{ok}` — legacy, авторизация не ужесточена (используется и sbe-requests, любым владельцем заявки); sbe-lims для Kanban-доски использует `kanban-move` ниже |
| POST | `/requests/{id}/kanban-move` | editor+ | `{status?, assigned_to?}` → `{request}` — Kanban-доска «Очередь лаборатории» (2026-08-24): руководитель лабы (admin/superadmin) — свободно; испытатель (`lab_operator`/`lab_admin` этой лабы) — только свою уже назначенную карточку между received/processing/completed, либо самозабор неназначенной `new` СЕБЕ (не кому-то другому); прочие — 403. См. `lab-service/AGENTS.md`, `kanban.go` (`canApplyKanbanMove`) |
...
| POST | `/methods` | admin, ИЛИ lab_admin ВСЕХ запрошенных `lab_ids` (2026-08-24) | `{code,name,lab_ids: number[],description?,determinable_indicators?}` → `{id}` (минимум одна лаба; метод может принадлежать нескольким, 2026-08-19) |
| PATCH | `/methods/{id}` | admin, ИЛИ lab_admin хотя бы одной из ТЕКУЩИХ лаб метода; при смене `lab_ids` — lab_admin ВСЕХ новых (2026-08-24) | `{formulas, classification, chart_configs, input_parameters, presentation, lab_ids?, description?}` (JSONB-поля — любой поднабор; `lab_ids`, если передан, полностью заменяет набор лабораторий; `description` — COALESCE, не перетирает при отсутствии; **`formulas` игнорируется, если передан `input_parameters`** — сервер перестраивает `formulas[]` целиком из атрибутов, см. модели ниже) → `{ok}` |
| DELETE | `/methods/{id}` | admin, ИЛИ lab_admin хотя бы одной из лаб метода (2026-08-24) | → `{ok}`; 409, если метод используется в заявках/справочниках |
...
LimsRequest{ id, number_seq, number_year, title, description, object_id, project_id, group_id,
             ...
             // Kanban-доска «Очередь лаборатории» (2026-08-24, см. kanban.go):
             // assigned_to — email испытателя (lab_members), назначает руководитель
             // лабы или сам испытатель (самозабор из "new" СЕБЕ); completed_at —
             // момент перехода в status="completed" (не updated_at), основа
             // 10-рабочедневного окна показа в колонке "Завершённые".
...
// sup/sub (2026-08-24) — взаимоисключающие верхний/нижний индекс, настоящий
// <sup>/<sub> (не юникод-символ) — реального contenteditable достаточно для execCommand.
InlineNode = { type: 'text', text?: string, bold?: boolean, italic?: boolean, sup?: boolean, sub?: boolean }
           | { type: 'placeholder', bold?: boolean, italic?: boolean, sup?: boolean, sub?: boolean,
...
// align (2026-08-24) — выравнивание, только paragraph/heading (список — свой маркер,
// таблицы — своя структура). static_table (2026-08-24) — визуальный конструктор:
// пользователь сам задаёт строки/столбцы И содержимое каждой ячейки (rows: строка →
// колонка → inline-содержимое, та же модель, что абзац) — в отличие от "table"
// (данные серий, авто-заполняемые столбцы из TableColumn).
RichNode = { type: 'paragraph' | 'heading', level?: 2|3|4, align?: 'center'|'right'|'justify', children: InlineNode[] }
         | { type: 'bullet_list', items: InlineNode[][] }
         | { type: 'table', columns: TableColumn[] }  // рендерится построчно по сериям эксперимента
         | { type: 'static_table', rows: InlineNode[][][] }
...
- **Редактор блока** (`block-editor.ts`, 2026-08-24): абзац/заголовок — выравнивание
  (`align`) + верхний/нижний индекс (настоящий `<sup>/<sub>` через
  `execCommand('superscript'/'subscript')`, как Ж/К); пункт списка (bullet_list) —
  перетаскивание для смены порядка (тот же паттерн, что у колонок таблицы); новый
  узел **static_table** — визуальный конструктор произвольной таблицы в тексте
  (число строк/столбцов + содержимое каждой ячейки задаёт пользователь, ячейка —
  mini rich-text) — отдельно от узла `table` (данные серий, авто-заполняемые
  столбцы).
...
**Первое сохранение серии результатов само двигает заявку** (сервер, `handleCreateResult`):
из `new`/`received` — в `processing` (2026-09-05), а если исполнитель ещё не назначен —
им становится тот, кто ввёл результаты (2026-09-12), при условии, что он `lab_operator`
или `lab_admin` этой лаборатории. Уже назначенного исполнителя это не меняет
(переназначение — только вручную, через `kanban-move`), `completed` автоматически не
переоткрывается. Исполнитель проставляется ДО смены статуса — уведомление в LPITrack о
взятии в работу берёт фамилию из того же значения.
...
Справочники испытателей/оборудования — editor+. **`lab_admin` — делегированный админ
СВОЕЙ лабы (2026-08-24, было — синоним `lab_operator` без реальных доп. полномочий):**
методы-конфиги (`POST /methods`/`PATCH`/`DELETE /methods/{id}`) и мутации `lab-members`
(`POST`/`DELETE`) — app-admin+ ИЛИ `lab_admin` ИМЕННО той лабы, к которой относится
операция (`requireLabAdminOf`/`OfAny`/`OfAll`, `lims_refs.go`) — метод может принадлежать
нескольким лабам, достаточно администрировать ОДНУ из них для PATCH/DELETE, но ВСЕ
запрошенные при создании/переназначении лаб. У `lab_admin` нет прав над ЧУЖИМИ лабами.
Чтение `lab-members` с `?lab_id=` — любой участник этой лабы (2026-08-24, см. выше).
Kanban-доска «Очередь лаборатории»: «руководитель лабы» = глобальная роль admin/
superadmin, ЛИБО `lab_admin` ИМЕННО этой лабы (2026-08-24, `canApplyKanbanMove`) —
свободное перетаскивание/назначение карточек; «испытатель» = `lab_operator` ИМЕННО
этой лабы (`kanban-move` проверяет через `labMemberRole`).
...
- Вьюха «LogicLAB.ЛИМС» (2026-08-21, было «Лабораторная информационная менеджмент
  ...
  - **Контент-карточка**: заголовок раздела + подзаголовок; **все 8 пунктов дерева
    ...
    - «Все заявки» / «Результаты и протоколы» (`completed`) — общий список (карточки)
      с **панелью фильтров** (2026-09-04, по образцу sbe-requests): дата создания
      «от/до», метод, статус, **важность** (Все / Обычный / Критический /
      Блокирующий — 2026-09-21), название объекта, идентификатор, партия,
      заказчик. Непустые поля комбинируются по И между собой и с
      caller-предиктом раздела (`matchesRequestFilters`); фильтрация целиком
      клиентская. Порядок задаётся фильтром статуса и одинаков во всех
      приложениях (2026-09-17, `sbe-core/src/utils/request-sort.ts`): «Все» — по
      `created_at` убыв., «Активные» — `processing` → `received` → остальные,
      внутри группы по `created_at` убыв., «Завершённые» — по `completed_at`
      убыв. (при пустом — `updated_at`). Раздел «Результаты и протоколы» получает
      режим `completed` явно (он весь из завершённых), выбранный руками статус
      этот режим перебивает. Раньше сортировка была по `number_year`/`number_seq`
      → карточка заявки. Номер в карточке списка —
      **сокращённый, лабораторный** `lab_number` (`{NNN}/{yyyy}-{methodCode}`), не
      полный номер заказчику (единственное место с коротким видом — везде остальном,
      включая заголовок карточки заявки/модалку протокола/имя файла DOCX, полный
      `customer_number`, 2026-08-24).
    - **«Очередь лаборатории»** (2026-08-24, было — тот же плоский список с фильтром
      `new`/`received`, статус `processing` вообще не показывался ни на одной
      специализированной странице) — **Kanban-доска** `renderQueueBoard()`, 4 колонки:
      Новые (`new`) / В работу (`received`) / В работе (`processing`) / Завершённые
      (`completed`). Колонки «В работу»/«В работе» делятся на ячейки по испытателям
      текущей лабы (`lab_members.role IN ('lab_operator','lab_admin')`) + хвостовая
      ячейка «Не назначено» (видна/принимает дроп только руководителю). Колонка
      «Завершённые» — плоский список, показывает заявку только 10 РАБОЧИХ дней (Пн–Пт)
      от `completed_at` (не `updated_at`), затем скрывает карточку с доски (заявка/статус
      в БД не трогается — видна на «Результаты и протоколы»). Перемещение — drag-and-drop
      ИЛИ контролы в карточке заявки (ниже), оба идут через один серверный
      `POST .../kanban-move`: руководитель лабы (глобальная роль admin/superadmin)
      двигает/назначает свободно; испытатель — только уже назначенные ему карточки между
      своими ячейками «В работу»⇄«В работе» и дальше в «Завершённые», либо самозабор
      неназначенной «Новой» СЕБЕ (не кому-то другому) — не может забрать для другого
      испытателя и не может переоткрыть завершённую.
      Порядок и важность (2026-09-17, прямой запрос пользователя): колонка «Новые
      заявки» сортируется `sortRequestsByPriority` (`sbe-core/src/utils/request-sort.ts`)
      — `blocker` → `critical` → остальные, внутри важности по `created_at` убыв.;
      остальные колонки порядок не меняют (там карточки разложены по испытателям).
      Верхняя полоса карточки (номер заявки) заливается по `priority` на ВСЕЙ доске:
      `blocker` — `--tn-err`, `critical` — `--tn-warn`, прочие — без заливки
      (`tn-lims-kanban-card-num--blocker/--critical`). Своей панели фильтров у
      доски нет — фильтр важности (2026-09-21) заведён только в списке заявок,
      здесь важность и так видна цветом.
    - Карточка заявки (статус/назначение исполнителя, ввод серии результатов, расчёт,
      таблица серий, графики, генерация протокола+DOCX). Показывает все те же детали,
      что видит заявитель в sbe-requests — номер заказчику, проект, группа, объект, ЕКН,
      внешний идентификатор (если есть, с восстановленным префиксом
      `LPIZAYAVKINAPRO-`), характеристики объекта (номер партии/идентификатор образца/
      целевой показатель — выбранное в sbe-requests значение для метода этой заявки),
      приоритет, цель испытания, лаборатория, владелец, даты создания/обновления,
      статус, **исполнитель** (2026-08-24, «не назначен» — пусто), описание — единым
      блоком сразу под номером. Статус/назначение (2026-08-24, заменило единый
      статус-select для любого `myRole !== ''`): у руководителя лабы — полный
      статус-select (4 значения) + select испытателя (ростер лабы, «не назначено»);
      у испытателя с неназначенной «новой» заявкой — кнопка «📥 Взять в работу»
      (самозабор); у испытателя со своей уже назначенной — ограниченный статус-select
      (`received`/`processing`/`completed`); у всех прочих — без контрола (раньше видел
    ...
    - «Сотрудники» — список (по текущей лабе) + добавление/удаление/**редактирование
      роли на месте** (`<select>`, 2026-08-24 — раньше смену роли можно было сделать
      только удалением и повторным добавлением) — admin+, ИЛИ `lab_admin` ИМЕННО
      текущей лабы (2026-08-24, делегированные полномочия — читает ростер через
      `?lab_id=`, доступно любому участнику своей лабы, не только admin, см. «Роли»
      выше); раздел скрыт, только если ни то ни другое.
...
- **Реализовано 2026-08-19**: бэкенд ролей (superadmin/lab_auditor/фильтрация `/labs`) —
  см. раздел «Роли» выше.
- **Реализовано 2026-08-24**: точная клиентская проверка `lab_operator`/`lab_admin`
  per-лаба для Kanban-доски — `GET /lab-members?lab_id=` (доступен любому участнику
  этой лабы, не только admin) + `fetchLabRoster`/`testersOf`/`myRoleIn` в `lims-view.ts`.
  Область действия на тот момент была ТОЛЬКО статус/назначение заявки; `canEdit`
  (результаты/расчёт) по-прежнему разрешает всё любой app-роли `!== ''`, сервер
  всё равно валидирует по `requireLabAccess`/`requireLabRead`.
- **Реализовано 2026-08-24**: делегированные полномочия `lab_admin` внутри своей
  лабы — методы (создание/конфиг/удаление своей лабы), «Сотрудники» (любая роль
  своей лабы), роль руководителя в Kanban-доске своей лабы — см. «Роли» выше.
  За пределами своей лабы у `lab_admin` прав нет.
