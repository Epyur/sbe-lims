...
- Вьюха «Лабораторная информационная менеджмент система СБЕ ПМиПИР» (тип `sbe-lims-view`,
  ...
  - **Контент-карточка**: заголовок раздела + подзаголовок. Пункт «Все заявки» наполнен
    ...
    серий, графики, генерация протокола+DOCX). Номер заявки показывается **сокращённым,
    лабораторным** — `lab_number` (`{NNN}/{yyyy}-{methodCode}`), НЕ полный номер заказчику
    (`customer_number`, с кодами проекта/лаборатории) — ЛИМС для сотрудников лаборатории,
    им не нужен код проекта. Остальные пункты — **заглушка** (наполняются следующими этапами).
