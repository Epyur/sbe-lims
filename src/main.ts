  ...
  async onload(): Promise<void> {
    ...
    if (this.settings.lastAnnouncedVersion !== this.manifest.version) {
      void this.announceUpdateSafely(
        'В разделе «Оборудование» внесён ряд улучшений: карточка прибора больше не ' +
        'закрывается и не сбрасывает введённые данные при сохранении или загрузке файлов; ' +
        'добавлен простой переключатель «Основное/Вспомогательное оборудование» прямо в ' +
        'карточке; список привязанного вспомогательного оборудования теперь виден сразу в ' +
        'общем перечне, без необходимости открывать карточку основного прибора (как в ' +
        'разделе «Документы»); появилось поле срока действия поверки и настраиваемые ' +
        'напоминания о его приближении.',
      );
    }
  }
