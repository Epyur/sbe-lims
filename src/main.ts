  ...
  async onload(): Promise<void> {
    ...
    if (this.settings.lastAnnouncedVersion !== this.manifest.version) {
      void this.announceUpdateSafely(
        'В конструкторе методов у агрегированных показателей появились два новых способа ' +
        'свода по сериям — специально для полей типа "Да"/"Нет": "Да, если хоть одна серия Да" и ' +
        '"Нет, если хоть одна серия Нет" (раньше для таких полей были только числовые среднее/' +
        'минимум/максимум, которые для Да/Нет не работали). Также исправлена ошибка: при загрузке ' +
        'файлов (в т.ч. фото из писем-результатов) повторная обработка того же файла могла ' +
        'создавать в заявке лишние дубликаты вложений — теперь повтор того же файла просто ' +
        'переиспользует уже загруженный, без дублей.',
      );
    }
  }
