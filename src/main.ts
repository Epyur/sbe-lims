  ...
  async onload(): Promise<void> {
    ...
    if (this.settings.lastAnnouncedVersion !== this.manifest.version) {
      void this.announceUpdateSafely(
        'При настройке формы для испытателя больше не предлагается показатель, ' +
        'который заполняется кнопками секундомера: вручную его вводить нельзя, ' +
        'а попытка это сделать стирала уже записанные наблюдения.',
      );
    }
  }
