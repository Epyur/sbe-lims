  ...
  async onload(): Promise<void> {
    ...
    if (this.settings.lastAnnouncedVersion !== this.manifest.version) {
      void this.announceUpdateSafely(
        'В настройках метода испытаний поля для испытателя теперь можно объединять ' +
        'в блоки: дать блоку название, отметить его обязательным и настроить, чтобы ' +
        'во второй и следующих сериях он был свёрнут, а данные подставлялись из ' +
        'предыдущей серии — общее для всех серий не нужно вводить заново.',
      );
    }
  }
