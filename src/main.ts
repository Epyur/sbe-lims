  ...
  async onload(): Promise<void> {
    ...
    if (this.settings.lastAnnouncedVersion !== this.manifest.version) {
      void this.announceUpdateSafely(
        'Заявка теперь сама переходит в статус «В работе», как только испытатель ' +
        'начинает вносить результаты — не нужно переставлять статус вручную. Также ' +
        'добавлена возможность настраивать поле для ввода кода прибора отдельно ' +
        'для каждого метода испытаний.',
      );
    }
  }
