  ...
  async onload(): Promise<void> {
    ...
    if (this.settings.lastAnnouncedVersion !== this.manifest.version) {
      void this.announceUpdateSafely(
        'Заявки в списке теперь идут в понятном порядке: без фильтра — по дате ' +
        'поступления, недавние сверху; в «Активных» — сначала те, что уже в ' +
        'работе; в «Завершённых» — недавно закрытые сверху. В очереди ' +
        'лаборатории новые заявки выстраиваются по важности: сначала ' +
        'блокирующие, затем критичные, затем обычные, а важность видна цветом ' +
        'верхней полосы карточки — красная и оранжевая. Справка переехала из ' +
        'отдельной заметки в окно, которое открывается прямо в приложении.',
      );
    }
  }
