  ...
  async onload(): Promise<void> {
    ...
    if (this.settings.lastAnnouncedVersion !== this.manifest.version) {
      void this.announceUpdateSafely(
        'Деления на шкалах графиков теперь всегда целые числа с округлым, читаемым шагом ' +
        '(0, 5, 10, 15... вместо дробных значений), подобранным так, чтобы подписи не наезжали ' +
        'друг на друга. Легенда графика переехала внутрь картинки, в правый верхний угол, и ' +
        'больше не занимает лишнее место над графиком. В визуальном редакторе протокола у ' +
        'статических (введённых вручную) таблиц теперь можно менять порядок столбцов ' +
        'перетаскиванием — как уже было у динамических таблиц с данными серий. Также исправлена ' +
        'ошибка: если у метода хотя бы одна расчётная формула была настроена некорректно, это ' +
        'молча блокировало ВСЕ остальные расчёты и всю классификацию по заявкам этого метода — ' +
        'теперь такая формула просто пропускается, а остальные считаются как обычно.',
      );
    }
  }
