  ...
  async onload(): Promise<void> {
    ...
    if (this.settings.lastAnnouncedVersion !== this.manifest.version) {
      void this.announceUpdateSafely(
        'Восстановлены реальные ЕКН и названия материала у 129 заявок, которые раньше ' +
        'ошибочно показывали один и тот же ЕКН (последствие объединения безымянных карточек ' +
        'материала). Включён тестовый приём результатов испытаний по почте (методы ГГ и РП) — ' +
        'без сбора всей почты, только выбранные заявки. Фотографии из письма-результата теперь ' +
        'можно вставить в протокол — в конструкторе они помечены значком 📷. Исправлена ошибка, ' +
        'из-за которой при трёх и более письмах по одной заявке пропадала итоговая статистика.',
      );
    }
  }
