  ...
  async onload(): Promise<void> {
    ...
    if (this.settings.lastAnnouncedVersion !== this.manifest.version) {
      void this.announceUpdateSafely(
        'В список заявок добавлена панель фильтров (по дате, методу, объекту, ' +
        'идентификатору, партии и заказчику). Настройки уведомлений о сроке поверки ' +
        'оборудования переехали из общих настроек плагина в настройки конкретной ' +
        'лаборатории — теперь каждая лаборатория может задать свои сроки и своих ' +
        'получателей, и управлять этим может её собственный администратор, а не только ' +
