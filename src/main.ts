  ...
  async onload(): Promise<void> {
    ...
    if (this.settings.lastAnnouncedVersion !== this.manifest.version) {
      void this.announceUpdateSafely(
        'В списке заявок появился отбор по важности — можно показать только ' +
        'обычные, только критические или только блокирующие. Он складывается с ' +
        'остальными фильтрами: например, критические среди активных. Сами ' +
        'слова важности теперь одинаковы во всех приложениях: «Обычный», ' +
        '«Критический», «Блокирующий». На доске «Очередь лаборатории» отбора ' +
        'нет — там важность и так видна цветом карточки.',
      );
    }
  }
