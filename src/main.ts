  ...
  async onload(): Promise<void> {
    ...
    if (this.settings.lastAnnouncedVersion !== this.manifest.version) {
      void this.announceUpdateSafely(
        'Показания прибора из письма-результата (температура, время экспозиции и т.п.) теперь ' +
        'сохраняются и доступны для построения графиков. В конструкторе методов появился новый ' +
        'тип показателя — "Временной ряд" — для таких данных. Графики научились рисовать ' +
        'заголовок, подписи осей и легенду (раньше эти надписи не отображались на картинке), а ' +
        'также накладывать два ряда разного масштаба на одно изображение со своей второй осью ' +
        '— например, температуру и скорость её нарастания вместе.',
      );
    }
  }
