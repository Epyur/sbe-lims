...
export interface Lab {
  ...
  /** Ручной адрес служебных писем (дубль в LPITrack при completed, уведомление о
   * взятии в работу) — приоритетнее общей настройки сервера (2026-08-29). Пусто —
   * используется настройка сервера. Адрес заказчика этого поля не касается. */
 ...
 * показатель метода. */
/** "heading_number" (2026-08-29, WP6, путь б) — не системное поле заявки и не
 * атрибут метода: сервер подставляет порядковый номер ЭТОГО БЛОКА среди
 * блоков (для текущего вида вывода — ui/excerpt/protocol), которые тоже
 * содержат этот плейсхолдер где-то в своём содержимом — счётчик общий на
 * блок (не на каждое вхождение), пользователь сам решает, куда его вставить
 * внутри вручную набранного текста (заголовок/абзац/пункт списка/ячейка). */
export type PlaceholderSource = 'system' | 'attribute' | 'heading_number';
...
export interface OperatorFormField {
  ...
  visibility?: OperatorFormVisibility;
  /** Группа формы, которой принадлежит поле (2026-09-11) — id из
   * MethodOperatorForm.groups; пусто у полей вне групп. Порядок полей в списке
   * не меняется: группа рисуется на месте своего первого поля, поля одной
   * группы идут подряд (конфигуратор переставляет их при назначении группы). */
  group_id?: string;
...
/** Условие показа поля или группы формы (2026-08-28, WP3c; вынесено в
 * именованный тип 2026-09-11 — теперь его носитель не только поле, но и
 * группа). field — id ДРУГОГО поля этой же формы. */
export interface OperatorFormVisibility {
  logic: 'and' | 'or';
  conditions: Array<{ field: string; operator: ComparisonOperator; value: string }>;
}

/** Группа полей формы испытателя (2026-09-11, по итогам боевого применения
 * ЛИМС Мобайл на заявке 451/2026 — часть полей общая на все серии, и
 * переписывать их в каждой серии заново испытателю незачем). Дизайн:
 * docs/superpowers/specs/2026-09-11-sbe-lims-operator-form-groups-design.md.
 * Сервер группы НЕ интерпретирует — это клиентская схема рендера, как timer и
 * visibility; наследование значений выполняет мобильный клиент при вводе,
 * поэтому в БД у каждой серии остаётся полный набор значений и вывод
 * документов не меняется. */
export interface OperatorFormGroup {
  /** Стабильный идентификатор, на него ссылается OperatorFormField.group_id. */
  id: string;
  /** Заголовок блока; он же подставляется в кнопку «Показать <title>». */
  title: string;
  /** Предупреждать при отправке, если внутри есть незаполненные поля
   * (предупреждение, а не запрет — испытание важнее полноты формы). */
  required?: boolean;
  /** Поведение во 2-й и последующих сериях. 'expanded' (по умолчанию) — как
   * обычный набор полей; 'collapsed_inherit' — блок свёрнут за кнопкой
   * «Показать <title>», значения наследуются от ПРЕДЫДУЩЕЙ серии и уходят на
   * сервер независимо от того, раскрывал испытатель блок или нет. */
  next_series?: 'expanded' | 'collapsed_inherit';
  /** Условие показа всей группы — та же форма, что у отдельного поля. Скрытая
   * по условию группа не отправляется вовсе (в отличие от свёрнутой). */
  visibility?: OperatorFormVisibility;
}
...
export interface MethodOperatorForm {
  ...
  /** Группы полей (2026-09-11) — см. OperatorFormGroup. */
